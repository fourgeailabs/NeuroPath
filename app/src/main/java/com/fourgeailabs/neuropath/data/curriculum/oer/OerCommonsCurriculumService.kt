...
import com.fourgeailabs.neuropath.data.curriculum.uk.UkNationalCurriculumCatalog
...
class OerCommonsCurriculumService(private val db: AppDatabase) {
    ...
    suspend fun initializeAndSeed(): List<OerCommonsCurriculumItem> = withContext(Dispatchers.IO) {
        ...
        if (count == 0) {
            ...
            val items = entities.mapNotNull { entity ->
                entity.toDomainModelOrNull() ?: run {
                    Log.w("OerCurriculumService", "Skipping unit with unrecognized subject/grade codes: id=${entity.id}")
                    null
                }
            }
    ...
    /**
     * Verifies the on-device curriculum library: any missing pre-installed units are inserted
     * without touching existing rows (non-destructive). OER Commons reachability is probed
     * for display purposes only — the app does not download collections, so the built-in
     * curated catalog remains the source of truth either way.
     */
    suspend fun verifyLocalCatalog(): OerSyncResult = withContext(Dispatchers.IO) {
        val dao = db.oerCurriculumDao()
        val preinstalled = PreinstalledOerCurriculumCatalog.getAllPreinstalledCurriculum()
        ...
        val reachable = runCatching {
            val request = Request.Builder()
                ...
                .header("User-Agent", "NeuroPath-K12-Educational-App/${com.fourgeailabs.neuropath.BuildConfig.VERSION_NAME}")
                .build()
        ...
        if (reachable) {
            OerSyncResult(
                isSuccess = true,
                sourceTitle = "Built-in Curriculum Library (Offline)",
                totalUnitsCount = count,
                message = "Verified $count curriculum units in the on-device library. OER Commons is reachable in your browser via the OER Hub button; the app does not auto-download collections."
            )
        } else {
            OerSyncResult(
                isSuccess = true,
                sourceTitle = "Built-in Curriculum Library (Offline)",
                totalUnitsCount = count,
                message = "Verified $count curriculum units in the on-device library. OER Commons could not be reached, so the built-in catalog remains the source of truth."
            )
        }
    }
